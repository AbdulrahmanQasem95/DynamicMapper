...
class ReadingWritingNestedPropertiesModel:DynamicCodable{
    ...
    var property_2:Int {dm.level1?.level2?.property2?.intValue ?? 0}
    ...
    var secondArrayItem_1_OfLevel_6:String {dm.level1?.level2?.level3?.level4?.level5?.level6Array?[1]?.item1?.stringValue ?? "item not found" }
